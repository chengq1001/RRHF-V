...
class CustomDataset(Dataset):
    def __init__(self, stage, dataset_desc_easy, dataset_desc_hard, dataset_pope, image_path, batch_size):
        ...
        print(f"Length before processing:{total_len}")
        adjusted_len = (total_len // batch_size) * batch_size
        self.data = total_data[:adjusted_len]
        print(f"Length after processing:{len(self.data)}")
...
if __name__ == "__main__":
    ...
    if extra_config.lora_path != "None":
        ...
        print(f"Successful merge:{extra_config.lora_path}!")
    ...
    trainer.train()
    end_time = time.time()
    print(f"The training time is:{(end_time - start_time)/3600}h")
